...
- Read protocol: use the runtime-equivalent Engram read flow defined by the contract; for example, `mem_search` followed by `mem_get_observation` when those tools exist (search previews are not sufficient).
- Write protocol: use the runtime-equivalent Engram save flow defined by the contract; for example, `mem_save` with exact `topic_key`, `scope: project`, `capture_prompt: false`, and structured content.
...
4. Route to the next required workflow-private subagent using only `project`, `run_id`, topic-key references, resolved `refactor_mode` when entering the refactor quality phase, and the protocol reminder: read dependencies through the contract's runtime-equivalent Engram read flow, write artifacts through the contract's runtime-equivalent save flow using exact topic keys, load the subagent's own required skills, and return a compact envelope only.
