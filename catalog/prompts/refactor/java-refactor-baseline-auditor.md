...
Follow the `java-refactor-engram-contract` skill for runtime-specific Engram tool mapping: read through the contract's read flow (for example, `mem_search` followed by `mem_get_observation` when available), and write through the contract's save flow (for example, `mem_save`) using exact `topic_key`, `scope: project`, `capture_prompt: false`, and structured content. Keep artifacts compact (gate status, result summaries, blockers, risks, next action). Return only the compact envelope.
