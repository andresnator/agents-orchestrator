...
name: skill-registry
description: >
  Create or update the skill registry for the current project. Scans user skills and project conventions, then writes .atl/skill-registry.md.
...
This is the foundation for runtime-agnostic skill resolution. The registry is built ONCE (expensive), then read cheaply whenever an orchestrator launches sub-agents.
...
1. Glob for `*/SKILL.md` files across ALL known skill directories. Check every path below — scan ALL that exist, not just the first match:
   ...
   **Project-level (workspace skills):**
   - `{project-root}/catalog/skills/*/*/SKILL.md` — Agents Orchestrator catalog
...
2. Include SDD workflow skills under `catalog/skills/sdd/`; they are first-class catalog skills.
3. **SKIP `skill-registry`** — that's this skill
...
- Include SDD workflow skills; skip only the `skill-registry` directory when scanning
