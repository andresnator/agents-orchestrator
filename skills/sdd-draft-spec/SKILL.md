...
- One file per capability: `specs/{capability}/spec.md`. Standalone writes default to `.ai/orchestrator/changes/`; a phase agent may receive a validated `.ai/{owner}/changes/` target. New and modified capabilities both use delta sections; new behavior goes under ADDED Requirements.
...
6. If standalone and approved, ask before writing `.ai/orchestrator/changes/{change-name}/specs/{capability}/spec.md`. When loaded by a phase agent, write only its validated owner-specific target root.
