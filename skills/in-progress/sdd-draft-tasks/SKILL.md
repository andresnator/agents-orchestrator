...
Use when drafting an OpenSpec `tasks.md` from an approved spec and design: an ordered, dependency-aware checklist. Works standalone or delegated by `grill-me-sdd`.
...
- Keep the artifact under 650 words. When delegated by grill-me-sdd, return the approved draft and do not write files; the orchestrator owns the single write step.
