...
- New `Work` groups add `Skills: <comma-separated names | none>`. Use names, never paths, and include only skills that materially affect implementation. Missing legacy fields default to `code-conventions` for code/test work and `none` otherwise.
...
- Every new work group names its implementation skills or `none`.
