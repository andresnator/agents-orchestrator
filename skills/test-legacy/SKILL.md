...
name: test-legacy
description: |
  Testing techniques for legacy code based on "Working Effectively with Legacy Code" by Michael Feathers.
  Use this skill whenever the user needs to write tests for existing code without coverage, break
  dependencies to make code testable, apply Characterization Tests, use Sprout/Wrap techniques to
  safely add functionality, deal with Singletons/statics/final classes in tests, find Seams in code,
  apply Sensing or Separation to isolate behavior, use TDD in legacy contexts, or any situation where
  legacy code hinders testing. Covers JUnit 4, JUnit 5, Mockito, and Java versions from 8 through 21+.
...
Battle-tested techniques for introducing tests into legacy code, based on Michael Feathers' "Working Effectively with Legacy Code."
