...
8. **Reconcile from receipts, then verify targeted.** The receipts carry most of the self-check: all four artifacts exist (`path`/`paths` present), marker first line correct and no kickoff line (`first_line`), forecast guard lines and per-group scopes present (`forecast_guards`, `files_scopes`), and in roadmap mode the `Roadmap: <goal> | Slice: <n>/<total>` second line correct (`second_line`) in the bundle drafted this sitting (already-written slice bundles are exempt — never edit their lines). A failed receipt assertion goes back to that phase agent as a correction brief. Then verify targeted instead of re-reading the bundle: read `tasks.md` only — the artifact the orchestraitor machine-consumes — checking tasks name real files, and fix minor inconsistencies yourself.
