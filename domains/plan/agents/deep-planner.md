...
- **Executable goal** (feature, change, bugfix) → a **ready-for-sdd bundle** under `.ai/deep-planner/changes/<change>/`, drafted by delegating to the sdd phase subagents, that the sdd `orchestraitor` validates and executes in place. This is the default path — see `## Bundle workflow`.
