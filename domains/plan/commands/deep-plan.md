...
- Output by goal:
  ...
  - **Oversized executable goal** → an ordered slice roadmap at `.ai/roadmaps/<goal>.md` plus a bundle for the next unblocked slice only, per the agent's Roadmap workflow and `docs/plan-handoff.md`.
  - **Decision / investigation** → a single Fable plan document following the `fable-planning` template.
- Allowed runtime write paths: `.ai/deep-planner/changes/**` (bundles), `.ai/roadmaps/**` (roadmaps), and `.ai/deep-planner/plans/**` (plan documents) only.
