...
For `judgment`, load `judgment-day`. Light uses `jd-solo`, fixes only CRITICAL findings once, and never re-judges. Full uses blind A/B judges, synthesis, authorized fixes, bounded re-check, and escalation. `verdict-only` never edits. Ask authorization questions directly.
...
On completion, lead with the review verdict, then give the ledger or changed paths and any user follow-up. Findings are `<path:line> <critical|warning|note> <problem>; fix=<intent>` plus one totals line. Explain blockers directly; omit logs, diffs, artifact bodies, commits, and push.
