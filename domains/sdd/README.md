...
Assumes the `common` domain is installed: the transversal `judgment-day`, `native-question-ux` (every user-facing question in the flow), `code-conventions`, `work-unit-commits` (commit shape under `Delivery: commit-per-wave`), and `tcr` (opt-in commit cadence for `sdd-implement`) skills live there. The orchestraitor's interview follows the `grilling` style inline (one question at a time, recommendation attached) without loading that skill, so `grilling` is not on any sdd allowlist.
