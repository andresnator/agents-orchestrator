...
3. Kickoff-lite: adopted bundles carry no kickoff line. Ask that one round via `native-question-ux` (skip anything the user already stated), record the kickoff line with `Depth: full` in `proposal.md`, and never re-ask. Adopted bundles are always full depth; do not ask Depth or offer light.
