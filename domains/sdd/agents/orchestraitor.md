...
- **Explore**: delegate to `sdd-explore` when the area is unknown or large; read inline when the change is bounded. Graph availability is not yours to manage: the `graphify-init` plugin builds and refreshes `.ai/graphify-out/graph.json` on its own, and no brief ever authorizes an agent to run Graphify lifecycle commands.
