...
- **Explore**: delegate to `sdd-explore` when the area is unknown or large; read inline when the change is bounded. Graph availability is not yours to manage: `.ai/graphify-out/graph.json` is first built via the human-run `/graphify-index` command and kept fresh by the `graphify-init` plugin, and no brief ever authorizes an agent to run Graphify lifecycle commands.
