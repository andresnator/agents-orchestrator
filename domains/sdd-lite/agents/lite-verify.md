...
Cold-check the exact scenarios in the brief. Require existing `change.md`, scenario ids, implementation scope, check, and `working-tree` or explicit diff range. Missing/contradictory input is `BLOCK sdd/lite-verify <reason>`; never infer paths.
