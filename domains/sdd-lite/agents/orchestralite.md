...
5. **Verify cold**: delegate to `lite-verify` with a complete brief: the `change.md` path, scenario ids, implementation scope, validation command, and diff range (`<baseline-sha>..HEAD` if the user had you commit; otherwise the working tree). Require its Output receipt. Reconcile the scenario-id set, PASS rows, empty `gaps`/`blockers`, and the exact clean token `VERIFY: ALL PASS — <n>/<n> scenarios.` as the receipt's first line. That spelling, period, and first-line position are authoritative because they match `lite-verify`'s Output contract; never ask the verifier to move the token to the final line or remove `scenarios.`. Re-delegate one genuinely malformed receipt naming the discrepancy; a second malformed receipt returns `failed`.
