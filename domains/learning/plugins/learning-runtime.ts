...
// Historical consent purposes from retired card operations stay readable in old states.
const HISTORICAL_PURPOSES = [...PURPOSES, "cards", "grade", "retirement", "reformulation", "override"]
const WRITER_OUTPUT_CONTRACT = "Return only JSON: {kind, source_revision, destination_hint, content, module_id?}. Compose one complete artifact; load exclusively method. Quote the learner only from learner_evidence references with their exact provenance; teaching_assessment, practice_status and approved_outline are teacher authorship, never learner quotes. Empty learner_evidence means evidence stays pending. Preserve supplied ownership fields."
...
// Retention survives only as an optional historical field of old schema-1 states.
...
const EVENT_TYPES = [
  "create_topic", "revise_scope", "select_module", "record_class", "start_practice", "skip_practice", "record_attempt", "record_consolidation", "close_module",
  "add_language_unit", "record_language_attempt",
...
const EVENT_REFERENCE: Record<EventType, EventReference> = {
  ...
  add_language_unit: { event: { type: "add_language_unit", event_id: "EVENT_ID", date: "YYYY-MM-DD", unit_id: "L-####", passive_at: "YYYY-MM-DD", situation: "string", target_text: "string", native_text: "string" }, rules: ["passive_at is a historical exposure record; units are available for practice from the learner's next choice, with no waiting period or calendar"] },
  record_language_attempt: { event: { type: "record_language_attempt", event_id: "EVENT_ID", date: "YYYY-MM-DD", unit_id: "L-####", outcome: "needs-another-attempt | completed | input-only", evidence: "actual learner evidence" }, rules: ["input-only keeps required production pending; needs-another-attempt records the error without scheduling a date"] },
...
interface TopicState {
  ...
  cards?: Array<{ id: string; cue: string; answer: string; concept_id: string; source_revision: number; box: number; last: string; next: string; status: string; again_count: number; lineage: string[] }>
...
function validStoredState(value: unknown, slug: string): value is TopicState {
  ...
  // Card, retention, and review fields are optional historical data of retired spaced repetition.
  for (const key of ["cards", "card_changes", "review_events"] as const) {
  ...
  if (!uniqueIDs(state.concepts) || !uniqueIDs(state.modules) || !uniqueIDs(state.language_units) || !uniqueIDs(state.vocabulary.candidates) || !uniqueIDs(state.gaps) || !uniqueIDs(state.jobs)) return false
  if ((state.cards !== undefined && !uniqueIDs(state.cards)) || (state.card_changes !== undefined && !uniqueIDs(state.card_changes))) return false
  ...
  if (state.review_events !== undefined && (!state.review_events.every((item: any) => record(item) && EVENT_ID.test(item.event_id) && cardIDs.has(item.card_id) && isIsoDate(item.date) && ["Again", "Hard", "Good", "Easy", "Again+reformulate", "Again+split"].includes(item.grade) && typeof item.evidence === "string" && item.evidence.trim()) || new Set(state.review_events.map((item: any) => item.event_id)).size !== state.review_events.length)) return false
  ...
  if (!state.consents.every((item: any) => record(item) && typeof item.interaction_id === "string" && (HISTORICAL_PURPOSES as readonly string[]).includes(item.purpose) && typeof item.request_id === "string" && /^[a-f0-9]{64}$/.test(item.digest) && strings(item.selected)) || new Set(state.consents.map((item: any) => item.interaction_id)).size !== state.consents.length) return false
...
// the host tool helper is resolved lazily through one loader function.
...
function initialState(slug: string, event: Extract<LearningEvent, { type: "create_topic" }>): TopicState {
  ...
  for (const concept of concepts) {
    ...
    if (concept.fundamental !== undefined) requiredBoolean(concept.fundamental, "fundamental")
  ...
  return {
    ...
    concepts: concepts.map((item) => ({ ...structuredClone(item), fundamental: item.fundamental === true, learner_override: false, taught: false })),
...
function createInteractions() {
  ...
  function stage(context: ToolContext, input: ChoiceInput): Choice {
    ...
    if (input.purpose === "mission") {
      ...
      if (input.multiple || !sameStrings([...ids], MISSION_OPTIONS)) throw new Error("invalid_choice_options")
...
function renderViews(state: TopicState): Record<string, string> {
  const concepts = state.concepts.map((item) => `| ${item.id} | ${markdown(item.title)} | ${item.prerequisites.join(", ") || "none"} | ${item.fundamental ? "yes" : "no"} | ${item.taught ? "yes" : "no"} |`).join("\n")
...
export const learningRuntimeContracts = { createInteractions, createJobs, createEvidence, applyLearningEvent, createStateStore, renderViews, normalizeVocabKey, localToday, isIsoDate }
      ...
      learning_commit: tool({
        ...
        async execute({ topic_slug, expected_revision, event }, context) {
          ...
          if (parsed.type === "attach_artifact") {
            ...
            if (output.selected_card_ids !== undefined) throw new Error("unexpected_writer_retention")
            if (output.module_id !== parsed.module_id) throw new Error("writer_owner_mismatch")
            if ((kind === "note" || kind === "exercise") && !output.module_id) throw new Error("writer_owner_mismatch")
          }
            ...
            if (input.worker === "learning-writer") {
              ...
              const teachingAssessment = owner
                ? {
                  ...
                  ...(owner.attempt?.evidence ? { attempt_assessment: owner.attempt.evidence } : {}),
                  ...(owner.consolidation?.learner_evidence ? { consolidation_assessment: owner.consolidation.learner_evidence } : {}),
                }
                : {}
              const practiceStatus = owner ? { skipped: owner.practice_skipped === true, outcome: owner.attempt?.outcome ?? null } : null
              input = { ...input, prompt: JSON.stringify({
                kind: artifact.kind, method: artifact.method, destination_hint: artifact.destination_hint, materials_language: artifact.materials_language,
                ...(artifact.module_id !== undefined ? { module_id: artifact.module_id } : {}),
                source_revision: input.revision, approved_outline: input.prompt,
                teaching_assessment: teachingAssessment, practice_status: practiceStatus, learner_evidence: learnerEvidence,
                output_contract: WRITER_OUTPUT_CONTRACT,
              }) }
            }
