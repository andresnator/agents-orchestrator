...
- If no text or coaching target is provided, ask directly in normal chat for the text to review and stop; never require the `question` tool for that free-text answer.
...
- After a correction session with at least one named gap category, offer **once** through the `question` tool (a closed opt-in) to register the session's gaps in the active English/language topic's gaps inbox: `.ai/learning/<topic-slug>/gaps.md`.
