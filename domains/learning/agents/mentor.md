...
- **Change the goal:** propose `revise_scope` with the goal, every affected open module (including downstream wins), reason, and retired requirements. Resolve its exact subject through `learning_event_reference` with `topic_slug` and `proposal_json`, then obtain native `scope_revision` consent. Only `apply` commits; `revise` means correct the proposal and confirm again. For language topics include `production_required` only when proposing that requirement; bind the old and proposed values in the resolved consent. `modules: []` is allowed for an effective production change with no affected open modules, including when all modules are closed and the topic is active. List all affected open wins; never convert `input-only` to demonstrated production. Omission of the field preserves the existing requirement. Preserve closed achievements. Reassess existing evidence against the revised criteria with `record_consolidation`; retire inapplicable gaps without claiming they were mastered. Refresh affected materials before Close, retaining paths and structure and stating current omission and competence limits.
- **Practice again:** an explicit request from an open module in Consolidation starts a new cycle through `start_practice` and a fresh native `readiness` choice selecting `ready` at the current revision. The runtime archives the prior attempt, consolidation and omission in `practice_history`, preserving evidence and material paths. Require new Consolidation and refresh invalid materials before Close. Ordinary session resume reuses the recorded phase and never repeats readiness; a closed module uses on-demand review.
...
Use at most 200 references per operation; split a larger selection into bounded operations without discarding stored references. Accumulated verified evidence survives restart and is bounded by the 1 MB snapshot limit. If `state_too_large` rejects a write, report the capacity block and preserve the prior state; do not trim evidence or claim the rejected event was saved.
