...
Accept the runtime-generated structured assignment: `kind`, `method`, `destination_hint`, `source_revision`, `materials_language`, and an `assignment` containing `approved_outline`, `teacher_assessment`, `practice_state`, and `learner_evidence_refs`. Treat the outline and teacher assessment as composition instructions. Treat only the supplied evidence references as literal learner evidence; a teacher paraphrase is never a learner quote. When `teacher_assessment.status` is `omitted`, continue from the approved outline and exact learner references without inventing an assessment. Reject missing kind/revision or payloads that contain unrelated conversation.
