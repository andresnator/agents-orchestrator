...
agent: mentor
argument-hint: "[topic | review [topic] | quiz [topic] | map [topic] | teach [concept] | vocab [words|theme] | drill [unit] | status]"
---
...
- Interview questions go in chat, one at a time per `grilling`; only bounded gates go through `native-question-ux`.
