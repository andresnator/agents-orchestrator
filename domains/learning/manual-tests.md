...
- **Steps:**
  1. Inspect `opencode debug agent mentor`, `opencode debug agent english-tutor`, `/doc`, and `/experimental/tool?provider=<fixture-provider>&model=<fixture-model>`. Invoke `learning_context`, `learning_event_reference` for `create_topic` and `grade_card`, and `recall_due` through the installed host.
  2. Stage the exact prompt `Save this card? Cue: Why revalidate? Answer: To check whether a stale response changed.` with `Save card` and `Save none`. Omit `multiple`, inspect the `not_shown` response, and call `learning_choice_result` once before showing anything. Both responses must direct `question` with exact `next_args`. Open it and reply `Save none` through the host endpoint or UI.
  3. Stage questions of exactly 300 and 301 characters: accept 300 unchanged and reject 301 before UI without truncation. Stage a short question with a large valid subject (over 300 characters); verify the subject and digest remain complete while `next_args` contains only the short question. Attempt to reuse consent for altered content. Replay that request ID, send an unrelated request ID, and stage a new choice before answering the old one. Settle 200 choices across sessions, then stage another.
  4. Launch a five-second scripted `learning-researcher` and verify `learning_job_start` stays pending until its terminal output. Attempt a writer or summarizer in the same parent while it runs; no second child may start. Continue with the returned result in the same turn. Launch another child, cancel the parent, and verify child settlement; simulate transport loss and recover the same ID with a waiting `learning_job_result`.
...
- **Expected result:** OpenCode, Node, and helper versions are recorded; both plugins and all intended `learning_*` and `recall_*` tools load. Invalid writer assignments (missing artifact/method, multiple artifacts, wrong destination, module, or selection) fail before child creation. Valid assignments carry canonical source_revision from revision. The event reference returns exact mission and repair choice IDs, the exact discriminated payload and consent subject without requiring Mentor to guess erased TypeScript types. An unshown choice reports `not_shown` with the exact next tool call; staging never claims the UI is open. Omitting `multiple` defaults to a single choice and the selected value is exactly `none`, bound to the shown display, session, call, and request; replay, forgery, stale replies, and entity substitution select nothing. Settled choices do not exhaust the pending-choice limit. `learning_context` announces `sequential_jobs`. The delayed tool call stays pending and returns the complete terminal result without a learner continuation; no deferred notice appears. Cancellation reaches the accepted child. Transport errors never prove completion or authorize an unchecked replacement. Foreign reads and broad writes fail; the test command is separately gated. Scripted timing is labeled protocol evidence, not model latency or teaching quality.
- **Essential negative variant:** Remove the host `session.prompt` method in a copied fixture and expect `sequential_session_api_unavailable`, with no child created. Remove the installed tool helper and expect `learning_tool_helper_unavailable`, not silently missing tools.
...
- **Steps:**
  ...
  5. Inspect `.ai/learning/<topic>/.state.json`, `mission.md`, `path.md`, `review-queue.md`, `vocabulary.md`, and `gaps.md`.
...
- **Steps:**
  ...
  2. At the post-Class card preview choose `Save none`. At the separate Practicar / Omitir readiness question, request clarification or dismiss it, then ask `How does ETag change the stale-response decision?`. Confirm `Practicar` through the native readiness choice.
  ...
  4. Explain the causal decision and one transfer case in your own words. Inspect the state and generated views after Close. Have the writer load `cornell-notes` with reads denied; check that the loaded skill includes the full lesson template. Repeat with native `Omitir` from Class and after a partial attempt; provide a brief explanation and compare all generated paths, sections, and table columns. Complete the final module against the agreed mission criteria, restart, and regenerate views.
- **Expected result:** The initialized fixture is read and resumed before any mission proposal. After Consolidation, Mentor automatically commissions and attaches the Cornell note, then commissions the learning-loop exercise at the new revision and attaches it before Close in the same turn, without a “continue” message or another materials request. Class contains the objective, central relationships, a different worked example, and one learner question. Card refusal records retention `none` without starting Practice. Clarification keeps the module in Class; the later readiness event starts Practice. The attempt and causal explanation are actual learner evidence and may satisfy Consolidation without redundant rituals. The module closes with no blocking gap and an empty selected-card list. Omission records optional `practice_skipped`, preserves partial attempts, and never fabricates completed practice. Resume reuses the recorded decision. Both materials still exist with omission in existing fields; no default capstone is imposed or practical competence credited without evidence. The writer receives the complete lesson template from the skill loader and requires no asset or project read. A writer result is reported as saved only after its exact source revision and content commit. Practice and Consolidation store verified `evidence_refs` from real learner messages. Mentor-only explanations never become learner quotes; teacher assessment remains separate. Sufficient evidence is reused without another exercise; only an uncovered criterion is asked. Sufficient mission evidence quotes, provenance, completion date, and event ID survive restart in `topic.completion` and the generated mission completion record.
...
- **Expected result:** The segment ends at the original save request. One bounded summarizer is awaited and saved with the retained approval in the same turn; later messages neither extend nor invalidate that segment. No continuation, extra save request, or deferred notice is needed. Exactly one concurrent call creates one localized Cornell summary under `.ai/learning/summaries/`; the other and later reuse fail with `summary_interaction_already_used`. A second explicit save gets a distinct collision-resistant path and does not overwrite the first. Neither file creates route state or cards.
...
- **Expected result:** Resolve the current card reference and copy exact `Again`/`Hard`/`Good`/`Easy` IDs with localized labels. Each valid grade takes one native confirmation and one fast revision-checked runtime commit; no per-grade model writer is launched. Only learner input advances cues. State and the queue show the same dates and boxes. Repair choices use the exact stored change ID as the positive option, never `accept`; translated labels are allowed. On the third cumulative failure, a plain grade is rejected until a correlated repair choice retires the old card and allocates new ID or IDs with lineage and fresh failure count.
...
- **Steps:**
  ...
  3. Add an escaped pipe in a cue, a duplicate active ID, an impossible Last/Next relation, a row missing its trailing `|`, and another malformed row, one at a time.
...
- **Steps:**
  1. Apply one event twice with the same event ID and body, then try that ID with a different body. Submit two distinct events concurrently at the same expected revision. Preview changes for two cards with the same `change_id`.
...
- **Steps:**
  1. In separate copies, select one card from two, select `Save none`, and select `Decide later` from the exact shown previews. Inspect the stored preview JSON and digest passed to each native choice, and compare the complete cue and answer shown in chat with that JSON; verify each native confirmation is at most 300 characters with brief visible options and no appended JSON.
  2. Choose `Edit C-0001`, commit and inspect the proposed cue and answer, then show the complete replacement cue and answer in chat and confirm briefly, binding its exact change JSON, ID, and digest. Repeat with a reformulation or split after two recorded `Again` events. In another copy, clarify the teaching so the source revision changes before answering the old preview.
  3. Replay an answered host request and submit a model-authored event containing `approved: true` but no correlated interaction. Use a grade choice whose subject names `C-0001` to submit `grade_card` for `C-0002`; repeat target substitution for readiness, status, override, export, and gap adoption.
  4. After selecting cards while still in Class, attempt a second `preview_cards` and a new `select_cards` against the old preview; inspect module ownership and the active queue.
  5. For a topic with fewer than five concepts, request an explicit learner override for one shown taught concept and inspect the shortlist.
- **Expected result:** Resolve the current module reference and copy proposal IDs, `none`, `deferred`, revision, subject and `multiple`. Generic `selected`, invalid option sets, and incompatible selection combinations cannot authorize writes; invalid options fail before UI. Retain the staged ID and exact `next_args`; valid save/refusal/edit needs one confirmation and one mutation. Final IDs are allocated only at commit for the exact selected proposals. None and deferred create no card and remain independent of module progress. Every durable choice requires the digest of its exact topic/entity subject; a choice for one target cannot mutate another. An edit uses purpose `cards`; reformulate/split use `reformulation`. Both require the stored change ID as the positive option and `cancel` as the negative option, with translated labels allowed. `accept` is rejected at staging. An edit or repair retires the former ID and creates confirmed replacement IDs with lineage. Changed teaching invalidates the old preview. Selected retention cannot be previewed or selected again; both attempts fail without changing card ownership or creating duplicate scheduled cards. Replayed, stale, partial, extra, and asserted approvals cannot save content. The default small-topic shortlist is zero; the explicit override is recorded without relabeling the default budget.
