...
- **Expected result:** OpenCode, Node, and helper versions are recorded; both plugins and all intended `learning_*` and `recall_*` tools load. The event reference returns the exact discriminated payload and consent subject without requiring Mentor to guess erased TypeScript types. An unshown choice reports `not_shown` with the exact next tool call; staging never claims the UI is open. Omitting `multiple` defaults to a single choice and the selected value is exactly `none`, bound to the shown display, session, call, and request; replay, forgery, stale replies, and entity substitution select nothing. Settled choices do not exhaust the pending-choice limit. The useful teaching question appears before the delayed child completes, learner input is accepted while it is busy, completion creates no unsolicited parent turn, and inspection returns the bounded final result. Cancellation settles the accepted child. Foreign reads and broad writes fail; the test command is separately gated. Scripted timing is labeled protocol evidence, not model latency or teaching quality.
...
- **Expected result:** Final IDs are allocated only at commit for the exact selected proposals. None and deferred create no card and remain independent of module progress. Every durable choice requires the digest of its exact topic/entity subject; a choice for one target cannot mutate another. An edit or repair retires the former ID and creates confirmed replacement IDs with lineage. Changed teaching invalidates the old preview. Selected retention cannot be previewed or selected again; both attempts fail without changing card ownership or creating duplicate scheduled cards. Replayed, stale, partial, extra, and asserted approvals cannot save content. The default small-topic shortlist is zero; the explicit override is recorded without relabeling the default budget.
