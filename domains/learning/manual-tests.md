...
- **Steps:**
  ...
  4. Explain the causal decision and one transfer case in your own words. Inspect the state and generated views after Close. Have the writer load `cornell-notes` with reads denied; check that the loaded skill includes the full lesson template. Complete the final module and capstone, restart, and regenerate views.
- **Expected result:** The initialized fixture is read and resumed before any mission proposal. After Consolidation, Mentor automatically commissions and attaches the Cornell note, then commissions the learning-loop exercise at the new revision and attaches it before Close, without another materials request. Class contains the objective, central relationships, a different worked example, and one learner question. Card refusal records retention `none` without starting Practice. Clarification keeps the module in Class; the later readiness event starts Practice. The attempt and causal explanation are actual learner evidence and may satisfy Consolidation without redundant rituals. The module closes with no blocking gap and an empty selected-card list. The writer receives the complete lesson template from the skill loader and requires no asset or project read. A writer result is reported as saved only after its exact source revision and content commit. Practice and Consolidation store verified `evidence_refs` from real learner messages. Mentor-only explanations never become learner quotes; teacher assessment remains separate. Sufficient evidence is reused without another exercise; only an uncovered criterion is asked. Capstone quotes, provenance, completion date, and event ID survive restart in `topic.completion` and the generated mission completion record.
- **Essential negative variant:** Submit fake message/part IDs, altered quotes, synthetic text, worker text and unstored foreign-session references; every rejection leaves state unchanged. Restart and complete with references already verified in the same topic, even with original history unavailable; replay the identical event without another write. Read old schema-1 states without promoting historical narrative or changing prior completion. Delay the writer, advance state with a valid learner event, then return the old output. The old artifact is rejected; teaching already supported by committed state may continue. A current-revision writer can later commit the artifact.
...
- **Expected result:** The segment ends at the original save request. Teaching continues while one bounded summarizer runs; later messages neither extend nor invalidate that segment. On the next real turn with a completed result, Mentor saves it with the retained approval without another save request. Completion produces no standalone parent turn; one correlated notice appears on a later real message. Exactly one concurrent call creates one localized Cornell summary under `.ai/learning/summaries/`; the other and later reuse fail with `summary_interaction_already_used`. A second explicit save gets a distinct collision-resistant path and does not overwrite the first. Neither file creates route state or cards.
