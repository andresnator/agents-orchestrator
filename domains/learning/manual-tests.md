...
Run each case independently from a new worktree and disposable OpenCode environment. Keep fixture validation, scripted protocol, model behavior, and human learning evidence separate. No model preset is authorized merely by this catalog.

## Quick path

1. Follow [per-ID preparation](../../manual-tests/fixtures/learning/README.md); use `prepare.py <ID> <new-absolute-directory>` from the assigned worktree, then `node manual-tests/fixtures/learning/validate.mjs <directory>`.
2. Install Learning separately in each listed variant (except the pre-copied standalone skills), source its `env.sh`, and start a fresh host session. The fixture manifest and preparation table are part of each case's preconditions.
3. Execute all numbered steps and negative variants with the [exact test inputs](../../manual-tests/fixtures/learning/materials/inputs.md). Record native choices, real session references, state diffs, effective models and evidence per assertion.
4. Archive evidence before stopping owned processes and cleaning PASS targets. Retain failed, blocked and resumable pending environments.

For multiple cases, use the [reusable Herdr execution prompt](../../manual-tests/learning-parallel-prompt.md): a continuous queue with up to four reusable panes, a new worktree/environment/agent session per assignment, progress review after 5 minutes and a 30-minute case timeout (both configurable). Human and future-date steps remain PENDING without occupying a pane.
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Stop the disposable host and provider. Remove their temporary project, config, and XDG roots; do not sync them to a global target.
...
- **Preconditions:** Prepare `MT-LEARNING-SESSION` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Start a fresh disposable project with no `.ai/learning/` directory.
- **Steps:**
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Close the session and remove the disposable project.
...
- **Preconditions:** Prepare `MT-LEARNING-PATH` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Start a fresh disposable project with the installed runtime available.
- **Steps:**
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Remove the disposable Learning state and project.
...
- **Preconditions:** Prepare `MT-LEARNING-MODULE-DELIVERY` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Use a disposable topic whose current module is `Freshness and validation` in Mission phase. No due cards, previews, or grades exist anywhere in the flow.
- **Steps:**
...
- **Preconditions:** Prepare `MT-LEARNING-INDEPENDENT-EVIDENCE` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Install only `learning` into a fresh disposable project and use the scripted loopback provider for protocol evidence; record real-model runs separately with their versions. Start a durable topic and reach Class without recording an attempt or consolidation.
- **Steps:**
...
- **Preconditions:** Prepare `MT-LEARNING-FLEXIBLE-PATH` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Use an isolated project with an active schema-1 topic. One module has skipped practice, verified theoretical explanations, materials, and a practical blocking gap; a later module also requires the practical deliverable. Do not use a real learner topic.
- **Steps:**
  ...
  5. In `production_open`, propose removing language production with all affected open wins; approve and reassess. In `production_closed`, remove production with `modules: []`, restart, verify completion eligibility, then reinstate production before completing the topic.
- **Expected result:** Production changes in the same revision as goal/wins, with exact before/after native consent. All closed modules retain closure and input-only outcomes never become production evidence.  Scope revision updates the goal and affected wins atomically with exact native consent. Old goals, wins, assessments, citations and retired requirements remain traceable. Retired practice is not described as mastered or still mandatory. Reassessment and current materials remain necessary for Close. Navigation persists the selected module, preserves deferred phases and evidence, and does not impose prerequisite gates or require materials. Resume honors the selection. Prior knowledge can reach Consolidation without a fictitious Class or practice; only actual verified learner evidence supports closure. Legacy snapshots remain readable.
- **Essential negative variant:** An altered production value or old consent must fail atomically. Reactivated production blocks completion while input-only evidence remains.  Cancel or alter either proposal, replay it against a newer revision, or substitute another module; state does not change. A changed scope with an old gap-free assessment still cannot close. A remaining conceptual gap blocks Close. Deferred modules block topic completion. A stale writer result cannot overwrite current material. Do not retry an already recorded omission or require repeated explanations merely after restart.
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Remove only the isolated project and target.
...
- **Preconditions:** Prepare `MT-LEARNING-AMBIGUOUS-ROUTE` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Use a disposable project with one initialized durable topic and a Spanish conversation.
- **Steps:**
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Close both sessions and remove the disposable project.
...
- **Preconditions:** Prepare `MT-LEARNING-SUMMARY-LIFECYCLE` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Complete a one-off session in a disposable project; no summary has been requested yet.
- **Steps:**
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Remove both generated summaries and the disposable project.
...
- **Preconditions:** Prepare `MT-LEARNING-REVIEW-ONDEMAND` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Use a disposable topic with at least one closed module whose note contains Cornell questions, and no pending topic lock.
- **Steps:**
  ...
  3. On the actual next day, run `/learn review <topic>` again (PENDING until then; release the pane) and confirm the second pass is identical in shape: self-contained questions, no accumulated calendar, no Leitner state.
...
- **Preconditions:** Prepare `MT-LEARNING-ENGLISH` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Use a disposable project. An initialized English topic is optional.
- **Steps:**
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Remove the disposable gap/topic and project.
...
- **Preconditions:** Prepare `MT-LEARNING-STANDALONE-SKILLS` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Prepare eight isolated config targets, each containing exactly one Learning skill directory and its own assets, with no Mentor, sibling skills, plugins, or `.ai/learning/` directory.
- **Steps:**
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Remove all eight isolated targets and projects.
...
- **Preconditions:** Prepare `MT-LEARNING-STATE-RECOVERY` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Use the scripted provider with an initialized disposable topic at a known revision.
- **Steps:**
  ...
  5. In `missing_worker`, accept a delayed child, abort it and delete only its case-owned host session while its persisted job is pending. Recover from another parent; test inspect, cancel and explicit retry. Inject a live topic lock during settlement to prove persistence failure keeps exclusion; remove only the owned lock and repeat recovery.
  6. Use `dead_claim` and `legacy_claim` with the fault helper; run the disposable lock-process regressions for publication interruptions, two recoverers and generation replacement.
  7. In `evidence_history`, submit 201 distinct real user excerpts in batches of at most 200, commit references, restart and reuse them. Separately reject a 201-reference operation and a snapshot over 1 MB, comparing prior state bytes.
- **Expected result:** A structured session.get 404/NotFoundError persists failed/worker_session_missing before an explicit replacement. Dead claim chains recover without removing active or replaced generations. Accumulated evidence exceeds 200 without trimming, remains reusable after restart and stays bounded by 1 MB.  Identical replay is a duplicate with no new IDs or revision; conflicting replay fails. One concurrent event wins, and every resulting snapshot validates. Recovery regenerates views from committed state without reapplying the event. A stale writer cannot attach, and another parent receives the existing pending child rather than launching a second one. A completed current writer survives process restart through its stored topic ownership and host session, and a new parent can commit its exact result. Foreign state is rejected before read. Exactly one process can claim a dead lock; live owners or ambiguous lock/claim records stop with no write; dead claim owners are recoverable.
- **Essential negative variant:** Timeouts, network/permissions failures, ambiguous errors and failed settlement writes keep jobs pending. Live claim owners or ambiguous records keep locks blocked. Record process checkpoints and distinguish deterministic fault injection from host protocol and model evidence.
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Stop the host and remove only the disposable state and config.
...
- **Preconditions:** Prepare `MT-LEARNING-NO-SRS-COMPAT` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Prepare copies of a legacy schema-1 topic whose state contains cards with boxes and dates, a module retention entry (pending and selected variants), review events, historical consents with retired purposes, and language units carrying `next_due`. Use only the supplied synthetic copies; no real learner topic or backup is required.
- **Steps:**
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Retain the synthetic historical comparisons in the report; never mutate real learner data.
...
- **Preconditions:** Prepare `MT-LEARNING-LANGUAGE-PRACTICE` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Create disposable language topics (production required) containing 1, 5, and 6 units passively seen today, with no `next_due` fields anywhere.
- **Steps:**
  ...
  5. In `revise_production`, natively approve retiring production with all modules closed and `modules: []`; confirm input-only outcomes survive. Reactivate production before completion, verify it blocks, then produce the unit and complete.
- **Expected result:** Scope revision changes the effective production gate without changing real unit outcomes or prior closures.  Practice availability follows the learner's choice and unit order, never a date or unit count. A material error records `needs-another-attempt` without scheduling a future date. Input-only stays valid and keeps required production pending until demonstrated. One-, five-, and six-unit courses drain in unit order with no buffer or invented unit. Completion is blocked until all production-required units are completed, and allowed once they are.
- **Essential negative variant:** Reject changed production proposals and missing/cancelled native consent.  Submit `add_language_unit` with `next_due`; the event fails with `unexpected_next_due` and the unit is not created. Attempt topic completion while a production-required unit is input-only; completion stays blocked with no invented mastery.
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Remove the three disposable topics and project.
...
- **Preconditions:** Prepare `MT-LEARNING-VOCAB-EXPORT` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Use a disposable English/Spanish topic with a known phrase inventory and no export for `check in`.
- **Steps:**
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Remove the disposable topic and batch.
...
- **Preconditions:** Prepare `MT-LEARNING-MODEL-VARIANCE` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. Obtain explicit credit authorization. Copy identical empty and initialized snapshots. Choose one inexpensive available model; choose one stronger model only under separate authorization. Record exact model IDs, timestamps, and costs.
- **Steps:**
  1. Run this ID’s own six prepared samples once per authorized model: ephemeral HTTP explanation, module readiness declined plus consolidation, two answered review questions, same-day language input-only, selected vocabulary export, and standalone Cornell. Use the exact sample sequences in the preparation table; do not execute another test ID or reuse its results.
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Delete only copied snapshots and disposable model sessions; preserve concise evidence outside learner state.
...
- **Preconditions:** Prepare `MT-LEARNING-DELAYED-RETENTION` with the [independent fixture instructions](../../manual-tests/fixtures/learning/README.md#preparation-by-id), including every listed variant. A learner completes one foundational module with a verified explanation and one transfer exercise on day 0. Record the actual dates as historical records; learner participation is required on days 7 and 30. No reminders, calendars, or scheduled events are created.
- **Steps:**
...
- **Cleanup:** Archive and verify the case evidence, stop owned services and end sessions before reusing the pane. Remove PASS copies; retain failed, blocked and resumable pending copies. Keep the learner-authorized durable record; remove only disposable copies used to inspect the case.
