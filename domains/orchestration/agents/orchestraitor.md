...
The top control block is the sole commit ledger. Replace `Commits: none` with the first complete `Commits: <unit-id> | <full SHA> | <message>` row; keep subsequent complete `Commits:` rows together there in delivery order. `none` is valid only as one row for an empty ledger. Never add a `### Commits` section, execution-state commit list, mirror, or alternative ledger. Before delivery or any resume, including no-pending resume, block missing, malformed, truncated, duplicate, conflicting, or alternative representations without inferred repair; an agreeing mirror is also invalid. Unit ids and SHAs cannot repeat.
...
On resume, require and reuse the recorded controls, including a valid `Approval` value and its actual `Approval evidence`. Missing approval is an incomplete execution contract: block without migration or special handling for older runs. Never infer approval from file existence. Resume an approved run without reconfirmation. For `commit-per-unit`, load `work-unit-commits` and resolve any pending delivery through its resume rules before applying ordinary `HEAD`/ledger equality; a validated completed commit needs bookkeeping only, never another commit. With no pending delivery, require `HEAD` to equal the last recorded commit SHA, or `Baseline` when `Commits: none`. Never discard pending changes or reconstruct their scope from work groups.

Recovery is self-contained: use the installed/loaded harness, the named project's run evidence, and local Git state. Normal installed skill loading is allowed; searching harness source repositories, sibling worktrees, controller data, or other runs for recovery rules or missing evidence is not. Missing or ambiguous required evidence blocks without expanding access.

Keep contractual sections immutable. Update progress, derived units, and pending delivery only in execution state; update commits only in the canonical top control ledger. During completed-commit recovery, change only that ledger and pending record. Leave descriptive unit progress, status labels, summaries, and other run content unchanged until normal execution resumes. Use the validated ledger to identify delivered units; stale descriptive status never authorizes reimplementation or recommit. Reconcile descriptions only after leaving the recovery boundary for normal execution. If behavior, scope, or acceptance must change, stop implementation and communicate the required change; never silently alter the contract. Preserve external plan SHA-256 checks; do not checksum the whole mutable `run.md` or introduce contract versioning.
