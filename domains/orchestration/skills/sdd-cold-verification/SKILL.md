...
3. Build separate scenario and check ledgers. For `working-tree`, inspect the scoped working-tree diff. For commit-per-unit, require the left side of the range to equal the full SHA on the run's `Baseline:` line and the right side to be literal `HEAD`; inspect only that range and the `Files:` scope, independently of the implementer's summary.
