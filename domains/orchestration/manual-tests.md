...
- **Steps:**
  ...
  5. In separate disposable copies with successful hooks and genuinely generated pending metadata/snapshots, request a controlled tool-boundary pause after the commit returns successfully but before ledger append (A); repeat after the complete canonical top control row append but before pending clear (B). Require separate bookkeeping operations so each boundary can be observed. Do not fabricate snapshots, manually manufacture the positive ledger state, or kill hooks. If either pause cannot be observed reliably, leave that variant pending, not passed.
  6. At each A/B pause, record full `HEAD`, commit count, complete `run.md` bytes, canonical ledger, pending metadata, snapshot hashes, index entries, and tracked/untracked worktree inventory, modes, hashes, and absences outside `.ai/`. Capture run-root file hashes as well. Start a fresh session with `continúa <exact-run-path>` and ask it to pause after pending recovery before normal execution, progress reconciliation, further units, or archive. Confirm fresh focused checks and protected-state rechecks; compare the recorded evidence. Inspect the exact run diff: A changes only top ledger rows and pending record; B changes only pending record. Descriptive unit status remains byte-identical, even when it still says pending.
  7. For both A and B, perform two subsequent fresh-session resumptions that stop at the same recovery boundary without normal execution. Require no extra rows or commits and byte-identical `run.md` and snapshot evidence on each no-pending resumption, especially B. Then allow normal execution and confirm stale descriptions do not cause redelivery of the recorded unit.
  8. Inspect conversation/tool events from recovery and every resumption for access paths. Require only the installed/loaded harness, project run evidence, and local Git state: normal installed skill reads are allowed, but harness source repository, sibling worktree, controller-data, and other-run searches or reads are absent. Do not grant broader filesystem access to make a test pass.
  9. In separate negative copies, remove a required pre-hook snapshot from a genuine A/B pause, then resume. Also introduce a contradictory `### Commits` section saying `none` beside a delivered top row, an agreeing mirrored section, a row outside top controls, and `Commits: none` mixed with complete rows. Test these ledger variants both with pending delivery and after pending has cleared. Record all evidence after setup and before resume; require a block with no recovery mutations or attempted external evidence lookup.
- **Expected result:** Plan execution and resume ask no duplicate contract or route approval. The run preserves `Approval: plan-execution` and its actual instruction as evidence. Delivery is serial. Pending evidence exists before staging and hooks, including when the first commit fails with `Commits: none`. Resume validates saved state, reruns the check, and commits the same unit before clearing pending state. Unrelated staged content and green commits remain intact; `.ai/`, hook bypass, history rewrites, and push stay absent. Cold verification uses exactly `<Baseline>..HEAD` before archive. Post-commit recovery appends one complete row only when missing, or clears pending only when the exact final row already exists. Both pauses recover without a new commit, staging, source changes, or snapshot replacement: HEAD, commit count, index/worktree evidence, and snapshot hashes stay invariant. Both subsequent resumptions add no row or commit and leave recovery evidence byte-identical. Only complete repeated top control rows represent multiple commits; no alternative sections or mirrors appear. Descriptive progress never overrides the ledger or changes during recovery. No external repository access occurs. Approval, immutable contract, and external plan checksum gates still apply.
- **Essential negative variant:** Change a pending target after rejection and confirm resume blocks without discarding it. A fresh SDD run with an already modified target also blocks without changing delivery. From separate copies of genuine post-commit pauses, test missing/incomplete metadata or snapshots; truncated, duplicate, or conflicting rows; wrong message, parent, changed paths, target blob/mode/deletion evidence, or unsupported entries; an extra HEAD commit or merge; target dirt or unrelated staged/tracked/untracked inventory, mode, hash, or absence drift; and a hook failure or unexpected hook mutation. Include a focused check that fails or changes protected state. Each blocks without inferred repair, regenerated snapshots, ancestry search, recommit, or Git/index/source mutation by recovery. Missing or ambiguous required evidence and malformed or alternative ledger representations also leave ledger, pending record, descriptions, and snapshots unchanged; this applies with or without pending delivery. Do not silently remove, reconcile, or prefer one ledger representation. Missing approval evidence or a changed external plan checksum still blocks before recovery bookkeeping.
