...
- **Steps:**
  ...
  6. At each pause, capture full `HEAD`, commit count, `run.md`, pending metadata, snapshot and run-root file hashes, index entries, and tracked/untracked inventory, modes, hashes, and absences outside `.ai/`. In a fresh session, request `continúa <exact-run-path>` and stop after recovery, before normal execution or archive. Inspect fresh focused checks, protected-state rechecks, and the exact run diff: A changes only top ledger and pending; B changes only pending. Descriptive status remains byte-identical.
  ...
  8. Inspect all recovery/resume events. Allow installed/loaded harness reads, project run evidence, and local Git only. Reject harness-source, sibling-worktree, controller-data, or other-run access; never broaden access to pass a test.
...
- **Expected result:**
  ...
  - Recovery: validate every ledger commit and parent. Persist one missing top row before a separate pending-clear write; an existing exact final row needs only pending clear. No new commit, staging, source change, snapshot replacement, or description update. HEAD/count and index/worktree/snapshot evidence stay invariant.
...
- **Essential negative variant:** Each condition below blocks without inferred repair, regenerated evidence, ancestry search for a substitute, recommit, or Git/index/source mutation by recovery:
  ...
  - Commit evidence: missing/incomplete metadata or snapshots; wrong message, parent, paths, target blob/mode/deletion evidence, unsupported entries, an extra HEAD commit, or merge.
  - Protected state: target dirt; unrelated staged/tracked/untracked inventory, mode, hash, or absence drift; a hook failure or unexpected mutation; a focused check that fails or changes protected state.
