...
- **Steps:**
  ...
  3. In separate copies, cancel at the native gate and interrupt before approval. On return, say `Retomo la sesión interrumpida; todavía no apruebo el contrato.` Inspect the re-presented contract and native choice before answering. Use an explicit contract approval for the positive chat variant; do not use an ambiguous `continúa` as evidence of either approval or refusal.
...
- **Title:** Resume serial SDD delivery before and after commit
...
- **Preconditions:** Prepare two Git-backed fixture copies with a green baseline, the complex plan under `.ai/`, its checksum, an unrelated staged `notes.txt`, an unrelated ignored `.manual-private.txt`, and executable `.git/hooks/pre-commit` copied from `hooks/reject-order-pricing-pre-commit`. Ignore Maven's generated `target/`; it must contain no tracked or unrelated user files. Use the skill's protected-state definition for every snapshot and comparison, including clean tracked and unrelated ignored files. Only the previously declared check outputs may vary.
- **Steps:**
  ...
  2. Inspect write/hook order and the pending record in `run.md`: unit id, source work ids, declared scope, changed paths, check, `Check outputs`, message, parent SHA, and pre-stage/pre-hook snapshots. Confirm `target/` was declared before the first focused check and recorded identically in both complete snapshots. Compare them with `HEAD`, index, and protected working tree before changing anything.
  ...
  5. In fresh copies with successful hooks, request a pause after commit success but before ledger write (A), then another after the ledger write but before pending clear (B). Pending metadata and snapshots must come from the real delivery. Observe separate completed writes; never fabricate positive evidence or kill hooks. An unobservable pause remains pending.
  6. At each pause, capture full `HEAD`, commit count, `run.md`, pending metadata, snapshot and run-root file hashes, and the complete protected state. Derive exclusions from the original check-output declaration, never from observed differences or all ignored paths. In a fresh session, request `continúa <exact-run-path>` and stop after recovery, before normal execution or archive. Require a fresh passing check even when its declared Maven reports change, unchanged protected state, and the exact run diff: A changes only top ledger and pending; B changes only pending. Descriptive status remains byte-identical.
  7. For A and B, resume twice more in fresh sessions, stopping at that same boundary. Require no extra rows or commits and byte-identical run/snapshot evidence. Then allow normal execution and confirm stale descriptions never cause redelivery.
  8. Inspect all recovery/resume events, including subsequent workers and shell searches. Allow installed/loaded harness reads and the named project only. Reject parent-directory, harness-source, sibling-worktree, controller-data, or other-run access; never broaden access to pass a test. In a negative copy, remove required local evidence while leaving a similarly named file outside the project; require a local block without searching for the external copy.
  9. In separate negative copies, remove the original pre-hook snapshot, or add a contradictory `### Commits` section, an agreeing mirror, a row outside top controls, or `Commits: none` mixed with complete rows. Test ledger variants both with pending delivery and after it clears. Capture evidence before resume; require a block without mutations or external evidence lookup.
  10. With multiple complete, unique rows, replace an earlier SHA with a nonexistent commit or an unrelated commit, retaining the correct final `HEAD` row. Test with and without pending delivery; require full-chain validation to block before trusting any delivered unit.
- **Expected result:**
  - Approval and contract: no duplicate approval; preserve `Approval: plan-execution`, its actual instruction, immutable contract, and external plan checksum.
  - Delivery: serial units; evidence precedes staging/hooks even with `Commits: none`; retry validates saved state and runs a fresh check. Preserve unrelated staged content and green commits. Exclude `.ai/` from commits; never bypass hooks, rewrite history, or push. Cold verification uses exactly `<Baseline>..HEAD` before archive.
  - Recovery: validate every ledger commit and parent. Persist one missing top row before a separate write of `Pending delivery: none`; an existing exact final row needs only that clear. Preserve enclosing headings. No new commit, staging, source change, snapshot replacement, or description update. HEAD/count, protected state, and snapshot evidence stay invariant; only declared check outputs may change during the focused check.
  - Repeated resume: no extra rows, commits, or recovery writes. Ledger, pending, descriptions, and snapshots remain byte-identical. No alternative ledger or external repository access appears.
- **Essential negative variant:** Each condition below blocks without inferred repair, regenerated evidence, ancestry search for a substitute, recommit, or Git/index/source mutation by recovery:
  - Precommit: a changed pending target after hook rejection, or a pre-existing modified target on a fresh SDD run. Preserve changes and delivery mode.
  - Commit evidence: missing/incomplete metadata or snapshots, including omitted clean files or a missing/inconsistent check-output declaration; wrong message, parent, paths, target blob/mode/deletion evidence, unsupported entries, an extra HEAD commit, or merge. Reject output declarations covering tracked or unrelated user files, or exclusions inferred after drift.
  - Protected state: target dirt; unrelated staged/tracked/untracked/ignored inventory, mode, hash, or absence drift; a hook failure or unexpected mutation; a focused check that fails or changes protected state. Mutating `.manual-private.txt` must block despite its ignored status.
  - Recovery gates: missing/ambiguous evidence, malformed/alternative ledger, invalid earlier ancestry, missing approval evidence, or changed plan checksum. Leave ledger, pending, descriptions, and snapshots unchanged, with or without pending delivery; never remove, reconcile, or prefer a representation.
