...
How external planners hand complete OpenSpec change bundles to the sdd `orchestraitor` for execution. `refactor-planner` is the first producer; any future planner (performance, migration, security) reuses the same shape.
