...
- If the project has `.ai/atl/skill-registry.md` (generated by the OpenCode `opencode-skill-registry` plugin), read it by its literal path. A valid snapshot has `## OpenCode Skills`, `## Agent Skills`, and `## Claude Skills`, each with `Description | Skill | Location` columns.
- Match assigned work against `Description` and select only names from `Skill`. Treat `Location` as diagnostic information only; never use it to read a skill body or bypass the runtime's native skill loader.
