...
| FLEXIBLE-PATH | `base`, `defer`, `cancel`, `stale`, `conceptual_gap` have two modules, skipped practice, materials and a practical gap. First submit Plugin theory input and record its verified explanation with `No implemented plugin.` still blocking. Execute the catalog's scope request, choose Apply; in `cancel` choose Cancel and then test changed subject. `stale` advances revision before using old approval. `conceptual_gap` retains `Cannot explain plugin activation permissions.` in reassessment. `defer` chooses the second module then explicitly returns to M-0001. `prior` starts in Mission: submit sufficient Cache explanation and choose Omitir, then request materials. |
| AMBIGUOUS-ROUTE | All copies contain initialized `pizza`. Say `/learn pizza`; choose localized one-off in `base`, durable in `durable`; dismiss and say `path de aprendizaje` in `dismissed`. For the one-off answer use `La fermentación produce gas que expande la masa.` |
